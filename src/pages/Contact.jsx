 ...
 const {alert, showAlert, hideAlert} = useAlert();
  ...
  const handleSubmit =(e)=>{
    ...
    emailjs.send(
      ...
      {
        from_name : form.name,
        to_name: "Amrit Romana",
        from_email: form.email,
        to_email: 'connectwithromana@gmail.com',
      ...
      import.meta.env.VITE_APP_EMAILJS_PUBLIC_KEY 
        ...
        showAlert({
          show:true,
          text:"I didn't not receive your message ",
          type:'danger'
        })
        setCurrentAnimation('idle');
        console.log(error);
  ...
  return (
    <section className='relative flex lg:flex-row flex-col max-container'>
     {alert.show && <Alert{...alert}/>}
      <div className='flex-1 min-w-[50%] flex flex-col'>
        ...
        <form onSubmit={handleSubmit} action="" className='w-full flex flex-col gap-7 mt-14'>
    ...
    <input type="email" name='email' className='input' placeholder='John@gmail.com' required value={form.email} onChange={handleChange} onFocus={handleFocus} onBlur={handleBlur}/>
      ...
      <label className='text-black-500 font-semibold'>
   Your Messgae
    <textarea  name='message' className='textarea' placeholder='Let me know how I can help you!' rows={4} required value={form.message} onChange={handleChange} onFocus={handleFocus} onBlur={handleBlur}/>
      </label>
      <button disabled={isLoading} type='submit' className='btn'onFocus={handleFocus} onBlur={handleBlur}>
    {isLoading ? 'Sending...' : 'Send Messgae'}
      </button>
        </form>
      </div>
